...
namespace NadekoBot.Modules.Administration
{
    ...
    public partial class Administration : DiscordModule
    {
        ...
        public async Task Kick(IUserMessage umsg, IGuildUser user, [Remainder] string msg = null)
        {
            ...
            {
                await channel.SendErrorAsync("I most likely don't have the permission necessary for that.").ConfigureAwait(false);
            }
        }
        ...
        {
            try {
                  await umsg.Channel.SendConfirmAsync("ℹ️ **Shutting down.**").ConfigureAwait(false);
            }
            catch (Exception ex) { 
                                    _log.Warn(ex);
            }
